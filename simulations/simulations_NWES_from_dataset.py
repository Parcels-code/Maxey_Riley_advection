...
we load the data into memory with xarray so that we can calculate the temporal
derivative using finite differences with a timestep that is 1/2 the timestep
of the underlying field (tested to give the smallest error)
...
def run_experiment(pt, rep,year,month,day,d):
    ...
    output_file_b = (output_directory + '{particle_type}/{loc}_'
                    'start{y_s:04d}_{m_s:02d}_{d_s:02d}_'
                    'end{y_e:04d}_{m_e:02d}_{d_e:02d}_RK4_'
                    'B{B:04d}_tau{tau:04d}_{land_handling}_cor_{coriolis}_gradient_{gradient}_loaded_time.zarr')

    output_file_Rep_b = (output_directory + '{particle_type}/{loc}_'
                    'start{y_s:04d}_{m_s:02d}_{d_s:02d}_'
                    'end{y_e:04d}_{m_e:02d}_{d_e:02d}_RK4_'
                    '_Rep_{Rep:04d}_B{B:04d}_tau{tau:04d}_{land_handling}_cor_{coriolis}_gradient_{gradient}_loaded_time.zarr')
    ...
    ds_ocean = xr.open_mfdataset(oceanfiles,preprocess = preprocess)
    # ds_ocean = ds_ocean.load()
    # data = {"U":ds_ocean.uo, "V": ds_ocean.vo}

    # dimensions = {"lon": ds_ocean.longitude, "lat": ds_ocean.latitude, "depth":ds_ocean.depth,"time": ds_ocean.time}
    # # print(ds_ocean)

    fieldset = FieldSet.from_xarray_dataset(ds_ocean, variables = variables, dimensions = dimensions, indices=indices,
                                            allow_time_extrapolation="False",mesh='spherical')
    ...
    # lon_particles = 4
    # lat_particles = 56
    times = np.zeros(nparticles) + 2 * delta_t
    ...
    if (particle_type == 'tracer'):
        ...
        kernels.append(MRAdvectionRK4_2D_drag_Rep_loaded_time)
        ...
        kernels.append(MRSMAdvectionRK4_2D_drag_Rep_loaded_time)
