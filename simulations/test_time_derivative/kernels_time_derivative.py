...
class InertialParticle2D(JITParticle):
    ...
    # Rep = Variable('Rep', dtype=np.float32, to_write=True, initial=0.)
...
def MRAdvectionRK4_2D(particle, fieldset, time):
    ...
    scheme for a 2nd order ODE equation. We use central finite differences
    for all derivaties meaning that we need to preload all timestamps at we
    need 3 time instances of the data to calculate these derivatives 
...
def MRAdvectionRK4_3D(particle, fieldset, time):
    ...
    integrated using the 4th order runge kutta scheme for a nonoverdamped
    equation. We appromate the derivative at t (1rst step rk4) with a forward
    time derivative and the derivative at t+delta_t (4th step rk4) with a
    backward time derivative.

    dependencies:
    - up, vp, wp, particle velocity (particle variables)
    - tau, stokes relaxation time particle (particle variable)
    - B, buoyancy particle (particle variable)
    - Omega_earth, angular velocity earth (fieldset constant)
    - g, gravitational acceleration (fieldset constant)
    - delta_x, delta_y, delta_z, delta_t step for finite difference method 
      gradients (fieldset constants)
    """
    tau_inv = 1. / particle.tau
    Bterm = (3. / (1. + 2. * particle.B))
    Bterm2 = 2 * (1 - particle.B) / (1 + 2 * particle.B) 
    w0 =  Bterm2 * fieldset.g * particle.tau
    norm_deltax = 1.0 / (2.0 * fieldset.delta_x)
    norm_deltay = 1.0 / (2.0 * fieldset.delta_y)
    ...
    # calculate time derivative of fluid field
    (uf_tp1, vf_tp1, wf_tp1) = fieldset.UVW[time + fieldset.delta_t, particle.depth,
                                            particle.lat, particle.lon]
    (uf_tm1, vf_tm1, wf_tm1) = fieldset.UVW[time - fieldset.delta_t, particle.depth,
                                            particle.lat, particle.lon]
    dudt1 = (uf_tp1 - uf_tm1) * norm_deltat
    dvdt1 = (vf_tp1 - vf_tm1) * norm_deltat
    ...
    # calculate time derivative of fluid field
    (uf_tp2, vf_tp2, wf_tp2) = fieldset.UVW[time1 + fieldset.delta_t,
                                            depth1, lat1, lon1]
    (uf_tm2, vf_tm2, wf_tm2) = fieldset.UVW[time1 - fieldset.delta_t, depth1, lat1, lon1]
    dudt2 = (uf_tp2 - uf_tm2) * norm_deltat
    dvdt2 = (vf_tp2 - vf_tm2) * norm_deltat
    ...
    # calculate time derivative of fluid field
    uf_tp3, vf_tp3, wf_tp3 = fieldset.UVW[time2+ fieldset.delta_t,
                                          depth2, lat2, lon2]
    uf_tm3, vf_tm3, wf_tm3 = fieldset.UVW[time2 - fieldset.delta_t, depth2, lat2, lon2]
    dudt3 = (uf_tp3 - uf_tm3) * norm_deltat
    dvdt3 = (vf_tp3 - vf_tm3) * norm_deltat
    ...
    # calculate time derivative of fluid field
    (uf_tp4, vf_tp4, wf_tp4) = fieldset.UVW[time3 + fieldset.delta_t,
                                            depth3, lat3, lon3]
    (uf_tm4, vf_tm4, wf_tm4) = fieldset.UVW[time3 - fieldset.delta_t,  depth3, lat3, lon3]
    dudt4 = (uf_tp4 - uf_tm4) * norm_deltat
    dvdt4 = (vf_tp4 - vf_tm4) * norm_deltat
...
def MRSMAdvectionRK4_3D(particle, fieldset, time):
    """
    Advection of particles using the slow manifold approximation of the Maxey-
    Riley equation in 3D without Basset history term and Faxen corrections. The
    equation is numerically integrated using the 4th order runge kutta  scheme
    for an overdamped equation (ODE order 1). We appromate the derivative at t
    (1rst step rk4) with a forward time derivative and the derivative at
    t+delta_t (4th step rk4) with a backward time derivative.
