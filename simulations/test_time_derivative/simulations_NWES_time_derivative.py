...
from kernels_time_derivative import InertialParticle2D, deleteParticle
from kernels_time_derivative import InitializeParticles2D
from kernels_time_derivative import MRAdvectionRK4_2D, MRAdvectionRK4_2D_Newtonian_drag, MRAdvectionRK4_2D_drag_REp, MRSMAdvectionRK4_2D_drag_REp
from kernels_time_derivative import MRSMAdvectionRK4_2D
from kernels_time_derivative import displace, set_displacement, measure_vorticity, measure_slip_velocity
from kernels_time_derivative import too_close_to_edge, remove_at_bounds
...
output_file_b = (output_directory + '{particle_type}/{loc}_'
                 ...
                 'B{B:04d}_tau{tau:04d}_{land_handling}_cor_{coriolis}_slip_velocity_central_time_derivative.zarr')
...
runtime = timedelta(hours=24) #  timedelta(days=30)
...
# startlat_release=51
# endlat_release=58
indices = { }# {'lon':np.arange(0,10,1).tolist(),'lat':np.arange(0,10,1).tolist(),'depth':[0]}
...
if (starttime >= start_new_dataset):
    ...
    input_filename = ('metoffice_foam1_amm15_NWS_CUR_'
                      ...
                      'hi{year_tplus:04d}{month_tplaus:02d}{day_tplus:02d}.nc')
...
# fieldset from xarray
ds = xr.open_mfdataset(oceanfiles, drop_variables = ['thetao','so','depth'])
fieldset = FieldSet.from_xarray_dataset(ds, variables, dimensions, allow_time_extrapolation="False" )
# fieldset = FieldSet.from_netcdf(oceanfiles, variables, dimensions, transpose =True,deferred_load = False, indices=indices,
#                                         allow_time_extrapolation="False" )#,chunksize={'U': {'time' : ('time', 3)}}) # use deferred load false to laod all the data (only test for very short sim please)
...
Delta_t = 2#3600 # 3o min or 3600 is a 1 hour
