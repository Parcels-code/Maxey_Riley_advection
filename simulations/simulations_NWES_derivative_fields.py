...
from kernels import InertialParticle2D, deleteParticle, InitializeParticles2D
from kernels_derivatives_fields import MRAdvectionRK4_2D_drag_Rep_derivatives_field,MRAdvectionRK4_2D_drag_Rep_constant_derivatives_field
from kernels_derivatives_fields import MRSMAdvectionRK4_2D_drag_Rep_derivatives_field, MRSMAdvectionRK4_2D_drag_Rep_constant_derivatives_field
...
def run_experiment(pt, rep,year,month,day,d):
    ...
    output_file_b = (output_directory + '{particle_type}/{loc}_'
                    'start{y_s:04d}_{m_s:02d}_{d_s:02d}_'
                    'end{y_e:04d}_{m_e:02d}_{d_e:02d}_RK4_'
                    'B{B:04d}_tau{tau:04d}_{land_handling}_cor_{coriolis}_gradient_{gradient}_derivatives_field.zarr')

    output_file_Rep_b = (output_directory + '{particle_type}/{loc}_'
                    'start{y_s:04d}_{m_s:02d}_{d_s:02d}_'
                    'end{y_e:04d}_{m_e:02d}_{d_e:02d}_RK4_'
                    '_Rep_{Rep:04d}_B{B:04d}_tau{tau:04d}_{land_handling}_cor_{coriolis}_gradient_{gradient}__derivatives_field.zarr')
    ...
    particle_type = pt # 'inertial_SM_Rep_constant'#'tracer_random'# 'tracer'# 'inertial_Rep_constant'#'inertial_drag_Rep'# 'inertial_drag_REp'#'inertial_SM_drag_REp' #'inertial_SM_drag_REp'#'inertial_SM_drag_REp'# 'inertial_drag_REp'
    # starting date 
    ...
    # load derivative fields
    derivatives_path = '/storage/shared/oceanparcels/output_data/data_Meike/NWES/derivatives/'
    derivatives_file_base = 'derivatives_field_{ys:04d}{ms:02d}{ds:04d}-{ye:04d}{me:02d}{de:04d}.nc'
    endtime_der = datetime(year=starttime.year,month=starttime.month,day=1) + timedelta(days=31)
    derivatives_file = derivatives_path + derivatives_file_base.format(ys = starttime.year,
                                                        ms = starttime.month, 
                                                        ds = 1,
                                                        ye = endtime_der.year,
                                                        me = endtime_der.month,
                                                        de = endtime_der.day)

    filenames_derivatives = {'dudt': derivatives_file,
                        'dvdt': derivatives_file,
                        'dudx':derivatives_file,
                        'dvdx':derivatives_file,
                        'dudy':derivatives_file,
                        'dvdy':derivatives_file}

    variables_derivatives = {'dudt': 'dudt',
                        'dvdt': 'dvdt',
                        'dudx':'dudx',
                        'dvdx':'dvdx',
                        'dudy':'dudy',
                        'dvdy':'dvdy'}
    dimensions_derivatives = {'dudt':{'lat':'latitude',
                                            'lon': 'longitude',
                                            'time': 'time_left'},
                                'dvdt':{'lat': 'latitude',
                                        'lon': 'longitude',
    ...
    fieldset_derivatives = FieldSet.from_netcdf(filenames_derivatives, variables_derivatives,dimensions_derivatives, mesh='spherical',allow_time_extrapolation="False")
    fieldset_derivatives.dudt.units = GeographicPolar()
    fieldset_derivatives.dudx.units = GeographicPolar()
    fieldset_derivatives.dudy.units = GeographicPolar()
    fieldset_derivatives.dvdt.units = Geographic()
    fieldset_derivatives.dvdx.units = Geographic()
    fieldset_derivatives.dvdy.units = Geographic()
    
    fieldset.add_field(fieldset_derivatives.dudt)
    fieldset.add_field(fieldset_derivatives.dvdt)
    fieldset.add_field(fieldset_derivatives.dudx)
    fieldset.add_field(fieldset_derivatives.dvdx)
    fieldset.add_field(fieldset_derivatives.dudy)
    fieldset.add_field(fieldset_derivatives.dvdy)

    
    # grid spacing for checking boundaries
    ...
    if (particle_type == 'tracer'):
        ...
        kernels.append(MRAdvectionRK4_2D_drag_Rep_derivatives_field)
    elif(particle_type == 'inertial_Rep_constant'):
        kernels.append(MRAdvectionRK4_2D_drag_Rep_constant_derivatives_field)
    elif(particle_type == 'inertial_SM_Rep_constant'):
        kernels.append(MRSMAdvectionRK4_2D_drag_Rep_constant_derivatives_field)
    elif (particle_type == 'inertial_SM_drag_Rep'):
        kernels.append(MRSMAdvectionRK4_2D_drag_Rep_derivatives_field)
