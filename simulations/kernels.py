...
def MRSMAdvectionRK4_2D_v2(particle, fieldset, time):
    """
    Advection of particles using the slow manifold approximation
    of the Maxey-Riley equation in 2D without Basset history term
    and Faxen corrections. The equation is numerically integrated
    using the 4th order runge kutta scheme for an overdamped equation.
    IN version 2 we use the rescaled coriolis parameter:
     Omega= 2 (1 - B) / (1 + 2 B) * f *tau which adds aditional terms
     for the coriolis force which are neglegted in the standard version, 
      as they are order tau^2 without the rescaling.
    (ODE order 1)
    ...
    # coriolis force
    f1 =  2 * fieldset.Omega_earth * math.sin(particle.lat * math.pi / 180)
    ucor1 = f1 * (-vf1)# - particle.tau * (-DvDt1 + uf1 * dudy1 - vf1 * dudx1))
    vcor1 = f1 * (uf1)# - particle.tau * (DuDt1 + uf1 * dvdy1 - vf1 * dvdx1))
    ...
    # coriolis force
    f2 =  2 * fieldset.Omega_earth * math.sin(lat1 * math.pi / 180)
    ucor2 = f2 * (-vf2) # - particle.tau * (-DvDt2 + uf2 * dudy2 - vf2 * dudx2))
    vcor2 = f2 * (uf2) # - particle.tau * (DuDt2 + uf2 * dvdy2 - vf2 * dvdx2))
    ...
    # coriolis force
    f3 = 2 * fieldset.Omega_earth * math.sin(lat2 * math.pi / 180)
    ucor3 = f3 * (-vf3) #- particle.tau * (-DvDt3 + uf3 * dudy3 - vf3 * dudx3))
    vcor3 = f3 * (uf3) # - particle.tau * (DuDt3 + uf3 * dvdy3 - vf3 * dvdx3))
    ...
    # coriolis force
    f4 = 2 * fieldset.Omega_earth * math.sin(lat3 * math.pi / 180)
    ucor4 = f4 * (-vf4) # - particle.tau * (-DvDt4 + uf4 * dudy4 - vf4 * dudx4))
    vcor4 = f4 * (uf4) # - particle.tau * (DuDt4 + uf4 * dvdy4 - vf4 * dvdx4))
