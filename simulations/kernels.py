...
def InitializeParticles3D(particle,fieldset,time): 
    ...
    w0 = 2 * (1 - particle.B) / (1 + 2 * particle.B) * fieldset.g * particle.tau
    particle.up=u
    particle.vp=v
    particle.wp=w+w0
    particle.uf_tm=u
...
def MRAdvectionRK4_2D(particle,fieldset,time): 
    ...
    (uf_tm1, vf_tm1) = fieldset.UV[time+particle.dt, particle.depth, particle.lat, particle.lon]
    ...
    # calculate spatial gradients fluid field
    (u_dxm2, v_dxm2) = fieldset.UV[time1, particle.depth, lat1, lon1-fieldset.delta_x]
    (u_dxp2, v_dxp2) = fieldset.UV[time1, particle.depth, lat1, lon1+fieldset.delta_x]
    (u_dym2, v_dym2) = fieldset.UV[time1, particle.depth, lat1-fieldset.delta_y, lon1]
    (u_dyp2, v_dyp2) = fieldset.UV[time1, particle.depth, lat1+fieldset.delta_y, lon1]
    ...
    # calculate spatial gradients fluid field
    (u_dxm3, v_dxm3) = fieldset.UV[time2, particle.depth, lat2, lon2-fieldset.delta_x]
    (u_dxp3, v_dxp3) = fieldset.UV[time2, particle.depth, lat2, lon2+fieldset.delta_x]
    (u_dym3, v_dym3) = fieldset.UV[time2, particle.depth, lat2-fieldset.delta_y, lon2]
    (u_dyp3, v_dyp3) = fieldset.UV[time2, particle.depth, lat2+fieldset.delta_y, lon2]
    ...
    # calculate spatial gradients fluid field
    (u_dxm4, v_dxm4) = fieldset.UV[time3, particle.depth, lat3, lon3-fieldset.delta_x]
    (u_dxp4, v_dxp4) = fieldset.UV[time3, particle.depth, lat3, lon3+fieldset.delta_x]
    (u_dym4, v_dym4) = fieldset.UV[time3, particle.depth, lat3-fieldset.delta_y, lon3]
    (u_dyp4, v_dyp4) = fieldset.UV[time3, particle.depth, lat3+fieldset.delta_y, lon3]
...
def MRAdvectionRK4_3D(particle,fieldset,time):
    ...
    (uf_tm1, vf_tm1, wf_tm1) = fieldset.UVW[time+particle.dt, particle.depth, particle.lat, particle.lon]
    dudt1 = (uf_tp1 - uf_tm1) * norm_deltat
    dvdt1 = (vf_tp1 - vf_tm1) * norm_deltat
    ...
    # calculate spatial gradients fluid field
    (u_dxm2,v_dxm2,w_dxm2) = fieldset.UVW[time1, depth1, lat1, lon1-fieldset.delta_x]
    (u_dxp2,v_dxp2,w_dxp2) = fieldset.UVW[time1, depth1, lat1, lon1+fieldset.delta_x]
    (u_dym2,v_dym2,w_dym2) = fieldset.UVW[time1, depth1, lat1-fieldset.delta_y, lon1]
    (u_dyp2,v_dyp2,w_dyp2) = fieldset.UVW[time1, depth1, lat1+fieldset.delta_y, lon1]
    (u_dzm2,v_dzm2,w_dzm2) = fieldset.UVW[time1, depth1-fieldset.delta_z, lat1, lon1]
    (u_dzp2,v_dzp2,w_dzp2) = fieldset.UVW[time1, depth1+fieldset.delta_z, lat1, lon1]
    dudx2 = (u_dxp2 - u_dxm2) * norm_deltax
    dudy2 = (u_dyp2 - u_dym2) * norm_deltay
    ...
    # calculate spatial gradients fluid field
    (u_dxm3,v_dxm3,w_dxm3) = fieldset.UVW[time2, depth2, lat2, lon2-fieldset.delta_x]
    (u_dxp3,v_dxp3,w_dxp3) = fieldset.UVW[time2, depth2, lat2, lon2+fieldset.delta_x]
    (u_dym3,v_dym3,w_dym3) = fieldset.UVW[time2, depth2, lat2-fieldset.delta_y, lon2]
    (u_dyp3,v_dyp3,w_dyp3) = fieldset.UVW[time2, depth2, lat2+fieldset.delta_y, lon2]
    (u_dzm3,v_dzm3,w_dzm3) = fieldset.UVW[time2, depth2-fieldset.delta_z, lat2, lon2]
    (u_dzp3,v_dzp3,w_dzp3) = fieldset.UVW[time2, depth2+fieldset.delta_z, lat2, lon2]
    dudx3 = (u_dxp3 - u_dxm3) * norm_deltax
    dudy3 = (u_dyp3 - u_dym3) * norm_deltay
    ...
    # calculate spatial gradients fluid field
    (u_dxm4,v_dxm4, w_dxm4) = fieldset.UVW[time3, depth3, lat3, lon3-fieldset.delta_x]
    (u_dxp4,v_dxp4, w_dxp4) = fieldset.UVW[time3, depth3, lat3, lon3+fieldset.delta_x]
    (u_dym4,v_dym4, w_dym4) = fieldset.UVW[time3, depth3, lat3-fieldset.delta_y, lon3]
    (u_dyp4,v_dyp4, w_dyp4) = fieldset.UVW[time3, depth3, lat3+fieldset.delta_y, lon3]
    (u_dzm4,v_dzm4, w_dzm4) = fieldset.UVW[time3, depth3-fieldset.delta_z, lat3, lon3]
    (u_dzp4,v_dzp4, w_dzp4) = fieldset.UVW[time3, depth3+fieldset.delta_z, lat3, lon3]
    dudx4 = (u_dxp4 - u_dxm4) * norm_deltax
    dudy4 = (u_dyp4 - u_dym4) * norm_deltay
...
def MRSMAdvectionRK4_2D(particle,fieldset,time):
    ...
    # calculate time derivative of fluid field
    (uf_tp1, vf_tp1) = fieldset.UV[time+particle.dt, particle.depth, particle.lat, particle.lon]
    (uf_tm1, vf_tm1) = fieldset.UV[time, particle.depth, particle.lat, particle.lon]
    dudt1 = (uf_tp1 - uf_tm1) * norm_deltat
    dvdt1 = (vf_tp1 - vf_tm1) * norm_deltat
...
def MRSMAdvectionRK4_3D(particle,fieldset,time):
    ...
    # calculate time derivative of fluid field
    (uf_tp1, vf_tp1, wf_tp1) = fieldset.UVW[time+particle.dt, particle.depth, particle.lat, particle.lon]
    (uf_tm1, vf_tm1, wf_tm1) = fieldset.UVW[time, particle.depth, particle.lat, particle.lon]
    dudt1 = (uf_tp1 - uf_tm1) * norm_deltat
    dvdt1 = (vf_tp1 - vf_tm1) * norm_deltat
    dwdt1 = (wf_tp1 - wf_tm1) * norm_deltat
