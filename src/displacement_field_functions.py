...
date: May 2025
author: Meike Bos
Functions to make displacement fields for paticles that come too close to the coast on A-grids (to prevend
stuck particles)
Using the tutorial of https://docs.oceanparcels.org/en/latest/examples/documentation_unstuck_Agrid.html  
"""
...
def make_landmask(fielddata : str) -> np.array:
...
def get_coastal_nodes(landmask):
    ...
    next to land. Computes the Laplacian of landmask. Dependencies:
    - landmask: the land mask built using `make_landmask`, where land cell = 1
                and ocean cell = 0.
    Output: 
    2D array array containing the coastal nodes, the coastal nodes are
...
def get_coastal_nodes_diagonal(landmask : np.array) -> np.array:
...
def get_shore_nodes_diagonal(landmask: np.array) -> np.array:
...
def create_displacement_field(landmask : np.array, double_cell : bool = False) -> np.array:
...
def distance_to_shore(landmask : Dataset, dx : float =1) -> np.array :
