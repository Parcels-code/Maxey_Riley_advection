...
def Mei1992_kernel(t, s, c1, c2, d, nu, Rep):
    tau_diff = d * d / nu
    term1 = (4 * np.pi * (t - s) / tau_diff) ** (1.0 / (2 * c1))
    fh = (0.75 + c2 * Rep) ** 3
    term2 = (Rep**3 * np.pi * (t - s) ** 2 / (fh * tau_diff**2)) ** (1.0 / c1)
...
def Daitche(f, N, h):
    ...
    KN = (
        ...
        * np.sqrt(h)
        * (
            (N - 1) * np.sqrt(N - 1, dtype=np.float128)
            - N * np.sqrt(N, dtype=np.float128)
            + 6 / 4 * np.sqrt(N, dtype=np.float128)
        )
        * f[0]
    )
    KSUM = np.sum(
        [
            4
            / 3
            * np.sqrt(h, dtype=np.float128)
            * (
                (k - 1) * np.sqrt(k - 1, dtype=np.float128)
                + (k + 1) * np.sqrt(k + 1, dtype=np.float128)
                - 2 * k * np.sqrt(k, dtype=np.float128)
            )
            * f[N - k]
            for k in range(1, N, 1)
        ]
...
def f_Mei1992(dudt, t, c1, c2, Rep, N, h, S):
    ...
    f = [
        ...
        * (
            1
            + (np.sqrt(np.pi * (t - h * k) ** 3 / (S**3)) * Rep**3 / (2 * (f_h) ** 3))
            ** (1 / c1)
        )
        ** (-c1)
