...
def buoyancy_drifter(diameter : float, heigth : float, mass : float, density_fluid : float) -> float:
...
def stokes_relaxation_time(diameter : float, kinematic_viscosity :float, buoyancy : float) -> float:
    """
...
def drag_length(diameter : float ,buoyancy : float,drag_coefficient :float = 0.45 ) -> float:
    """"
...
def find_rep_white1991(Rep : float, tau_coriolis : float, tau_tides :float ,buoyancy :float,kinematic_viscosity : float,uf :float,diameter : float) -> float:
    """
...
def find_rep_morrison2013(Rep : float ,tau_coriolis : float,  tau_tides :float,buoyancy:float,kinematic_viscosity:float,diameter:float) -> float:
    """
    funtion needed to find Reynolds kinematic_viscositymber (using find root)
    using the slow manifold maxey riley equation. To make sure 
    that the input REp is same as the calculated Rep.
...
def find_rep_schiller1933(Rep : float ,tau_coriolis : float,  tau_tides :float,buoyancy:float,kinematic_viscosity:float,diameter:float) -> float:
    """
    funtion needed to find Reynolds kinematic_viscositymber (using find root)
    using the slow manifold maxey riley equation. To make sure 
    that the input REp is same as the calculated Rep.
...
def measure_coriolis_force(particle, fieldset, time):
    ...
    particle.fcor_u = fcor_u * fieldset.Rearth * math.cos(particle.lat * math.pi / 180.) * math.pi / 180.
    particle.fcor_v = fcor_v * fieldset.Rearth * math.pi / 180. 
