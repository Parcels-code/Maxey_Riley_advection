...
def create_sea_mask_polygon(region: gpd.GeoDataFrame,  land_boundaries : gpd.GeoDataFrame, countries_name_engl : list) -> gpd.GeoDataFrame:
    """
...
def from_dataset_to_points(ds: xr.Dataset, T: int, sea_domain : gpd.GeoDataFrame ) -> list:
    """
    ...
    points = [geometry.Point(np.round(lon,5), np.round(lat,5)) for lon, lat in zip(ds.lon[:,T].values,ds.lat[:,T].values) if np.isnan(lon)==False and lon > lon_min and lon < lon_max and lat > lat_min and lat < lat_max]
    return points
...
def make_unique_seeds(points : list) -> gpd.GeoDataFrame:
    ...
    unique_seeds = gpd.GeoDataFrame({"geometry": points_unique},
    ...
    duplicates_df = pd.DataFrame(list(duplicates.items()), columns=["geometry", "duplicates"]) 
...
def make_regional_voronoi_tesselation(unique_seeds : gpd.GeoDataFrame ,sea_region : gpd.GeoDataFrame, epsg :int = 25832 ) -> gpd.GeoDataFrame:
    """
    ...
    voronoi_cells["area"] = voronoi_cells_in_meters.geometry.area/1E6/voronoi_cells["duplicates"]
    voronoi_cells["density"] = 1/voronoi_cells["area"]

    return voronoi_cells


def plot_voronoi(fig, ax, voronoi_cells : gpd.GeoDataFrame, color_scale_type : str  , colormap : colors.ListedColormap = cm.magma , vmin : float = 0 , vmax : float = 0, colormap_scale : str = 'log'):
